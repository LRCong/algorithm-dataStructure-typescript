let partition = (args, lo, hi, compare) => {
    let tmp = args[lo], i = lo, j = hi;
    while (i < j) {
        while (j > i && compare(args[j], tmp) > 0)
            j--;
        if (j > i)
            [args[i], args[j]] = [args[j], args[i]];
        while (i < j && compare(args[i], tmp) <= 0)
            i++;
        if (i < j)
            [args[i], args[j]] = [args[j], args[i]];
    }
...
let partition3way = (args, lo, hi, compare) => {
 ...
 */
let quick_sort = function (args, compare) {
